...
ElementType TypedArray<ElementType>::pop() {
    ...
    ElementType result = safe_get(size() - 1);
    end--;
    return result;
}
...
TypedArray<ElementType> TypedArray<ElementType>::concat(const TypedArray<ElementType>& other) {
    ...
    for (int i = 0; i < size(); i++) {
        result.set(result.size(), safe_get(i));        
    }
    for (int i = 0; i < other.size(); i++) {
        result.set(result.size(), other.safe_get(i));
    }
    return result;
}
...
void sort_by_magnitude(std::vector<double> &arr);
